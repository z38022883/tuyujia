...
将 `app/api/*/route.ts` 的 handler 抽离为独立 Express/Fastify 服务：

```
现：Next.js API Route → 调用 src/server/*
改：独立 Node.js 服务 → 复用 src/server/*
    ↑
    小程序通过 HTTPS 调用
```
...
|---|---|---|---|
| 纯 TS 逻辑 | 90% | **90%** | 保持 |
| 后端服务 | 80% | **75%** | API 抽离 + 域名配置 + 鉴权适配 |
| UI 组件 | 30% | **25%** | Tailwind 重写成本未充分计入 |
| 浏览器胶水层 | 10% | **10%** | 保持 |
| **整体** | 40-50% | **30-40%** | 偏向 30% |

### 4.2 时间估算（三人组，留 buffer）

| 阶段 | 之前估算 | 修正后 | 理由 |
|---|---|---|---|
| Phase 1 基建 | 2 周 | **2-3 周** | 加域名备案等待 |
| Phase 2 核心 | 3 周 | **4-5 周** | Tailwind 适配比预期难 |
| Phase 3 完善 | 2 周 | **3 周** | 同步联调 |
| Phase 4 收尾 | 1 周 | **2-3 周** | 真机测试 + 审核迭代 |
| **总计** | 8 周 | **11-14 周** | 约 3 个月 |
...
| 任务 | 估算 | 产出 |
|---|---|---|
| Next.js API Routes 抽离为独立服务 | 1 周 | Express/Fastify 服务 |
...
| 任务 | 估算 | 产出 |
|---|---|---|
| StorageAdapter 接口设计 | 3 天 | 接口定义 + 文档 |
| 本地存储实现（wx.storage） | 3 天 | LocalStorageAdapter |
| 云端存储实现（云数据库） | 1 周 | CloudStorageAdapter |
| Zustand stores 迁移 | 2 天 | stores 适配 |
| Utils 迁移（纯 TS 部分） | 3 天 | segment-text、matcher 等 |
| Providers 适配（TTS、NLG） | 1 周 | MiniProgramTTS、复用 NLG |
| Services / Repositories 改造 | 1 周 | sync-service 改造完成 |
| 测试迁移（纯逻辑部分） | 1 周 | Vitest 测试搬到小程序环境 |
...
Phase 1 — 基建搭建（Week 1-3）
  ...
  B: StorageAdapter 设计 + stores/utils/types 迁移
...
| **Web 版与小程序版代码分叉** | 双端维护成本高 | 抽离共享 monorepo 包（types/utils） |
...
1. **是否保留 Web 版本？** 还是 All-in 小程序？
   - 保留 Web：建议 monorepo 共享逻辑层
   - All-in 小程序：现有 Web 版可作为参考但不维护
2. **是否使用微信云开发？** 还是自建后端？
   - 云开发：省事，但锁定微信生态
